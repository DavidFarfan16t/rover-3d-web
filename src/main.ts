...
import {
  ANTI_WHEELIE_FALL_RATE,
  ANTI_WHEELIE_MIN_TORQUE_SCALE,
  ANTI_WHEELIE_RISE_RATE,
  AUTOPILOT_BRAKE,
  AUTOPILOT_MAX_THROTTLE,
  CLIMB_ENGINE_FORCE,
  CONTACT_RECOVERY_FORCE_PER_WHEEL,
  CRUISE_ENGINE_FORCE,
  FIXED_STEP,
  GEAR_RATIO,
  MANUAL_BRAKE,
  ...
  MAX_FRAME_DELTA,
  MAX_INDEPENDENT_ROCKER_ANGLE,
  MAX_SUBSTEPS,
  MAX_VISUAL_ROLL,
  MODEL_URL,
  MOTOR_COUNT,
  MOTOR_TORQUE_NM,
  NORMAL_MAX_DRIVE_SPEED,
  PHYSICS_HZ,
  START,
  SUSPENSION_REST,
  TERRAIN,
  ...
  TERRAIN_Z_MAX,
  TERRAIN_Z_MIN,
  THEORETICAL_WHEEL_FORCE_N,
  THROTTLE_FALL_RATE,
  THROTTLE_RISE_RATE,
  TRACTION_ASSIST_FALL_RATE,
  TRACTION_ASSIST_RISE_RATE,
  TURBO_MAX_DRIVE_SPEED,
  TURBO_TARGET_SPEED_KMH,
  WAYPOINT_PASS_RADIUS,
  WHEEL_RADIUS,
  WHEEL_TORQUE_NM,
} from "./config";
import {
  createMarsHorizonGround,
  createMarsSky,
  createMarsTerrain,
  periodicDelta,
  terrainHeight,
...
import type { DriveCommand, MissionBlock, Waypoint } from "./mission-types";
import {
  alignBallLink,
  applyBinding,
  applyRoverComponentColors,
  bindBallLink,
  bindControl,
} from "./rover-model";
import { ui } from "./ui";
...
async function start() {
  ...
  const wheels = ["WHEEL_FL", "WHEEL_FR", "WHEEL_RL", "WHEEL_RR"]
    .map((name) => bindControl(model, name, new THREE.Vector3(1, 0, 0)));
  const steeringControls = ["STEER_FL_CTRL", "STEER_FR_CTRL", "STEER_RL_CTRL", "STEER_RR_CTRL"]
    .map((name) => bindControl(model, name, new THREE.Vector3(0, 1, 0)));
  const suspensionControls = [
    bindControl(model, "SUSP_L_CTRL", new THREE.Vector3(1, 0, 0)),
    bindControl(model, "SUSP_R_CTRL", new THREE.Vector3(1, 0, 0)),
  ];
  const differentialControl = bindControl(model, "DIFF_CTRL", new THREE.Vector3(0, 1, 0));
  const differentialLinks = [
    bindBallLink(model, "LINK_L_CTRL", "LINK_L_TARGET"),
    bindBallLink(model, "LINK_R_CTRL", "LINK_R_TARGET"),
  ];
